...
> Point d'entrée pour reprendre le projet **sans contexte préalable**. Dernière MAJ : 2026-06-05.
...
- **Moteur** : graders `io` / `unit` / `norme` / **`mutation`** (Roslyn embarqué, refs via
...
- **Contenu** : modules **M00–M35** + **Rushes 0/1/2** (34 modules, 114 exercices).
...
- ⏳ **Restants** : grader **git** dédié (M05/M14, via LibGit2Sharp) → Point 4 ; **Rush 3** (worker complet) →
  Point 3 ; **harnais réseau** (M22) → Point 5. Ordre conseillé : 4 → 3 → 5.
