...
- **Tests** : 167 verts (`dotnet test Piscine.slnx -c Release` : Core 46 + App 1 + Components 1 + Git 7 +
  DevHost.E2E 1 + Grading 111). CI GitHub Actions verte sur `main`. (Avant v4 S1 : 164 ; +3 = App sanity +
  bUnit + Playwright smoke.)
...
## v4 — application desktop Photino (S1 FAIT ✅, S2 à suivre)
...
- ✅ **#22 S1 — Fondation** (branche `v4/s1-foundation`). RCL **`Piscine.Components`** (composants + services
...
- ⏭ **#23 S2** = spike **terminal PTY embarqué** (Pty.Net + xterm.js) — risque dominant suivant.
- **Suivis notés en S1 (pour S2+)** : (a) déplacer `Error.razor` (dépend de `HttpContext`, host-only) de la
  RCL vers `Piscine.DevHost` ; (b) enregistrer `CourseCatalog` dans le DI du host Photino avant d'y monter
  une vraie page routable (sinon throw au runtime — le spike S1 ne monte que `MarkdownView`).
