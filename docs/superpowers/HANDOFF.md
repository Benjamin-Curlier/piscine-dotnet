...
- **Tests** : 225 verts (`dotnet test Piscine.slnx -c Release` : Core 46 + App 39 + Components 16 + Git 7 +
  DevHost.E2E 6 + Grading 111). CI GitHub Actions verte sur `main`. (164 avant v4 ; +3 S1 ; +2 S2 ; +26 S3 ;
  +9 S4 ; +15 S5 ; +6 S6 = CourseToc bUnit 5 + E2E lecteur 1.)
...
## v4 — application desktop Photino (S1 → S6 FAITS ✅, S7 à suivre)
...
- ⏭ **#28 S7** = **init/setup in-app** (enrobe `piscine init` : initialiser le dépôt/workspace de la recrue depuis l'app).
