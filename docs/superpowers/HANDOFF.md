...
- **Tests** : 235 verts (`dotnet test Piscine.slnx -c Release` : Core 46 + App 45 + Components 19 + Git 7 +
  DevHost.E2E 7 + Grading 111). CI GitHub Actions verte sur `main`. (164 avant v4 ; +3 S1 ; +2 S2 ; +26 S3 ;
  +9 S4 ; +15 S5 ; +6 S6 ; +10 S7 = InitService 6 + InitPanel bUnit 3 + E2E /init 1.)
...
## v4 — application desktop Photino (S1 → S7 FAITS ✅, S8 à suivre)
...
- ⏭ **#29 S8** = **surveillance du résultat de push** (`grade-received`) : l'app surveille le résultat éducatif
  écrit par le hook après `git push` et le rend richement (cf. spec §6 « rendu officiel »).
