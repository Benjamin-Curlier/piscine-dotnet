...
- **Tests** : 169 verts (`dotnet test Piscine.slnx -c Release` : Core 46 + App 2 + Components 1 + Git 7 +
  DevHost.E2E 2 + Grading 111). CI GitHub Actions verte sur `main`. (164 avant v4 ; +3 en S1 = App sanity +
  bUnit + Playwright smoke ; +2 en S2 = PtyService echo + smoke terminal.)
...
## v4 — application desktop Photino (S1 + S2 FAITS ✅, S3 à suivre)
...
- ⏭ **#24 S3** = moteur **statut git + coaching** (shim git + état dépôt, pas de parsing stdout) dans `Piscine.App`.
