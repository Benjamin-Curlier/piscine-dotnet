...
- `src/Piscine.Cli` : orchestration/affichage (`AssemblyName=piscine`). Commandes :
  `list`, `start <exo>`, `check <exo>`, `status`, `init`, `grade-received <sha>`,
...
1. Écrire les fichiers du/des module(s).
2. `&$env:PISCINE_CONTENT="$PWD/content"; dotnet run --project src/Piscine.Cli -- validate-content`
   → doit afficher **« Contenu valide. »** (compile chaque corrigé via Roslyn + vérifie l'E/S).
3. `git commit` puis `git push origin main` (**appels séparés** — un refus du push perd le commit).
4. CI : `gh run list --branch main --limit 1 --json status,conclusion`.
