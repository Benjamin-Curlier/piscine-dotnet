...
- **Tests** : 195 verts (`dotnet test Piscine.slnx -c Release` : Core 46 + App 27 + Components 1 + Git 7 +
  DevHost.E2E 3 + Grading 111). CI GitHub Actions verte sur `main`. (164 avant v4 ; +3 S1 ; +2 S2 ; +26 S3
  = GitStatusService + CoachingService 1/règle + GitPathResolver + shim relay + E2E coaching.)
...
## v4 — application desktop Photino (S1 → S3 FAITS ✅, S4 à suivre)
...
- ⏭ **#25 S4** = **`check` instantané in-process** (`Piscine.Grading` en process) + rendu du feedback (diff
  attendu/obtenu + indices + `course_ref`), **sans git** — remplace la sortie console de `piscine check`.
- **Suivis notés (pour sprints suivants)** : (a) déplacer `Error.razor` (dépend de `HttpContext`, host-only)
  de la RCL vers `Piscine.DevHost` ; (b) enregistrer `CourseCatalog` dans le DI du host Photino avant d'y
  monter une vraie page routable (sinon throw au runtime — le spike S1 ne monte que `MarkdownView`) ;
  (c) confirmer le terminal sur Linux/macOS (checklist retex S2) ; (d) coalescer la sortie PTY si verbeuse.
